...
type GocnRpcServer interface {
	Register(name string, service interface{})
	Run()
	Stop()
}

type GocnTcpServer struct {
	host           string
	port           int
	listen         net.Listener
	...
	RegisterCli    register.GocnRegister
	LimiterTimeOut time.Duration
	Limiter        *rate.Limiter
}

func NewTcpServer(host string, port int) (*GocnTcpServer, error) {
	listen, err := net.Listen("tcp", fmt.Sprintf("%s:%d", host, port))
	if err != nil {
		return nil, err
	}
	m := &GocnTcpServer{serviceMap: make(map[string]any)}
	m.listen = listen
	m.port = port
	m.host = host
	return m, nil
}
func (s *GocnTcpServer) SetLimiter(limit, cap int) {
	s.Limiter = rate.NewLimiter(rate.Limit(limit), cap)
}
func (s *GocnTcpServer) Register(name string, service interface{}) {
...
type GocnTcpConn struct {
	conn    net.Conn
	rspChan chan *GocnRpcResponse
}

func (c GocnTcpConn) Send(rsp *GocnRpcResponse) error {
	if rsp.Code != 200 {
		//进行默认的数据发送
	}
	//编码 发送出去
...
func (s *GocnTcpServer) Run() {
	for {
		...
		msConn := &GocnTcpConn{conn: conn, rspChan: make(chan *GocnRpcResponse, 1)}
...
func (s *GocnTcpServer) readHandle(conn *GocnTcpConn) {
	defer func() {
	...
	if msg.Header.MessageType == msgRequest {
		if msg.Header.SerializeType == ProtoBuff {
			...
			rsp := &GocnRpcResponse{RequestId: req.RequestId}
...
func (s *GocnTcpServer) writeHandle(conn *GocnTcpConn) {
	select {
...
func (s *GocnTcpServer) SetRegister(registerType string, option register.Option) {
	s.RegisterType = registerType
	s.RegisterOption = option
	if registerType == "nacos" {
		s.RegisterCli = &register.GocnNacosRegister{}
	}
	//if registerType == "etcd" {
	//	s.RegisterCli = &register.GocnEtcdRegister{}
	//}
}

func decodeFrame(conn net.Conn) (*GocnRpcMessage, error) {
	//1+1+4+1+1+1+8=17
	headers := make([]byte, 17)
...
func NewTcpClient(option TcpClientOption) *GocnTcpClient {
	return &GocnTcpClient{option: option}
}

func (c *GocnTcpClient) Connect() error {
	var addr string
...
func (c *GocnTcpClient) Close() error {
	if c.conn != nil {
		return c.conn.Close()
	}
	return nil
...
func (c *GocnTcpClient) Invoke(ctx context.Context, serviceName string, methodName string, args []any) (any, error) {
	...
	rspChan := make(chan *GocnRpcResponse)
	go c.readHandle(rspChan)
	rsp := <-rspChan
	return rsp, nil
}

func (c *GocnTcpClient) readHandle(rspChan chan *GocnRpcResponse) {
	defer func() {
		if err := recover(); err != nil {
			log.Println("GocnTcpClient readHandle recover: ", err)
			c.conn.Close()
		}
	}()
	for {
		...
		if msg.Header.MessageType == msgResponse {
			if msg.Header.SerializeType == ProtoBuff {
				...
				rsp1 := &GocnRpcResponse{}
				json.Unmarshal(marshal, rsp1)
				rspChan <- rsp1
			} else {
				rsp := msg.Data.(*GocnRpcResponse)
				rspChan <- rsp
			}
			return
		}
...
func (c *GocnTcpClient) decodeFrame(conn net.Conn) (*GocnRpcMessage, error) {
	//1+1+4+1+1+1+8=17
	headers := make([]byte, 17)
	_, err := io.ReadFull(conn, headers)
	if err != nil {
...
type GocnTcpClientProxy struct {
	client *GocnTcpClient
	option TcpClientOption
}

func NewGocnTcpClientProxy(option TcpClientOption) *GocnTcpClientProxy {
	return &GocnTcpClientProxy{option: option}
}
func (p *GocnTcpClientProxy) Call(ctx context.Context, serviceName string, methodName string, args []any) (any, error) {
	client := NewTcpClient(p.option)
	client.ServiceName = serviceName
	if p.option.RegisterType == "nacos" {
		client.RegisterCli = &register.GocnNacosRegister{}
	}
