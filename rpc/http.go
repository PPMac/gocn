...
type GocnHttpClient struct {
	client     http.Client
	serviceMap map[string]GocnService
}

func NewHttpClient() *GocnHttpClient {
	...
	return &GocnHttpClient{client: client, serviceMap: make(map[string]GocnService)}
}

func (c *GocnHttpClient) GetRequest(method string, url string, args map[string]any) (*http.Request, error) {
...
func (c *GocnHttpClient) FormRequest(method string, url string, args map[string]any) (*http.Request, error) {
...
func (c *GocnHttpClient) JsonRequest(method string, url string, args map[string]any) (*http.Request, error) {
...
func (c *GocnHttpClientSession) Response(req *http.Request) ([]byte, error) {
	return c.responseHandle(req)
}

func (c *GocnHttpClientSession) Get(url string, args map[string]any) ([]byte, error) {
	//get请求的参数 url?
	if args != nil && len(args) > 0 {
		url = url + "?" + c.toValues(args)
	}
...
func (c *GocnHttpClientSession) PostForm(url string, args map[string]any) ([]byte, error) {
...
func (c *GocnHttpClientSession) PostJson(url string, args map[string]any) ([]byte, error) {
...
func (c *GocnHttpClientSession) responseHandle(request *http.Request) ([]byte, error) {
...
func (c *GocnHttpClient) toValues(args map[string]any) string {
	if args != nil && len(args) > 0 {
		params := url.Values{}
		for k, v := range args {
...
type GocnService interface {
	Env() HttpConfig
}

type GocnHttpClientSession struct {
	*GocnHttpClient
	ReqHandler func(req *http.Request)
}

func (c *GocnHttpClient) RegisterHttpService(name string, service GocnService) {
	c.serviceMap[name] = service
}

func (c *GocnHttpClient) Session() *GocnHttpClientSession {
	return &GocnHttpClientSession{
		c, nil,
	}
}
func (c *GocnHttpClientSession) Do(service string, method string) GocnService {
